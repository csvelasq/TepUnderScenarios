...
class PowerSystemScenario(object):
    ...
    def import_scenarios_from_excel(excel_filepath, system=None):
        ...
        if system is None:
            imported_system = PowerSystem.import_from_excel(excel_filepath)
        else:
            imported_system = system
        imported_scenarios = []
        # import definition of scenarios (consisting of a name only)
        df_scenarios_definition = pd.read_excel(excel_filepath, sheet_name="ScenariosDefinition")
        ...
        for index, row in df_scenarios_definition.iterrows():
            ...
            state = PowerSystemState(imported_system,
                                             row['StateName'],
                                             row['Duration'])
            scenario.states.append(state)
        # import load scenario data
        df_scenarios_load_data = pd.read_excel(excel_filepath, sheet_name="ScenarioLoadData")
